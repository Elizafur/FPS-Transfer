...
public class GunController : MonoBehaviour
{
    ...
    void SwitchWeaponHandler()
    {
        ...
        if (gunSelected())
        {
            //NOGUN SHOULD ALWAYS BE EQUIPPED IN SLOT 0
            float scroll = input.mouseScroll;
            if (scroll >= 0.1f) ChangeSelectedGun(1);
            else if (scroll <= -0.1f) ChangeSelectedGun(2);
            if (player.GetComponent<PlayerInput>().wepOne)
            {
                if (selectedGun == 1)
                    return;
            ...
            else if (player.GetComponent<PlayerInput>().wepTwo)
            {
                if (selectedGun == 2)
                    return;
    ...
    void ChangeSelectedGun(int number)
    {
        ...
        try{
        if (gunInventory[number] == null)
            return;
        } catch (ArgumentOutOfRangeException e)   {
            selectedGun = putAwayGun;
            putAwayGun = -1;
            return;
        }
        selectedGun = number;
